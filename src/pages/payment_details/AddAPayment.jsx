...
export default function AddAPayment() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) {
        setFailedToSaveMessage(`Failed to add payment details: ${data.message}`);
        setFailedToSaveMsgOpen(true);
        setPageLoading(false); 
        return;
      }
      setSaveSuccessfulMessage("Payment details added successfully! You will now be redirected to All Payments Page.");
      setSaveSuccessfulMsgOpen(true);
    } catch (err) {
      console.error(err);
      setFailedToSaveMessage(`Failed to add payment details: ${err.message}`);
      setFailedToSaveMsgOpen(true);
    } finally {
      setPageLoading(false); 
    }
        ...
        <div className="bg-white rounded-2xl shadow p-6 max-w-6xl mx-auto">
          <form onSubmit={handleSubmit} className="flex flex-col items-center space-y-4 max-w-6xl mx-auto">
            ...
            <div className="flex flex-row items-center justify-center gap-4">
              ...
              <div className="flex gap-6 w-[210px]">
                <label className="flex items-center gap-2">
                  <input type="radio" checked={isDiscountAvailable} onChange={() => setIsDiscountAvailable(true)} className="accent-[#1E293B] cursor-pointer" />
                  Yes
                </label>
                <label className="flex items-center gap-2">
                  <input type="radio" checked={!isDiscountAvailable} onChange={() => setIsDiscountAvailable(false)} className="accent-[#1E293B] cursor-pointer" />
                  No
                </label>
              </div>
            </div>
            ...
            <div className="flex flex-row items-center justify-center gap-4">
              ...
              <TextField id="discount_amount" value={formData.discount_amount} onChange={handleChange} variant="outlined" disabled={!allowWriteAccess || !isDiscountAvailable} className={`w-[210px] ${!allowWriteAccess || !isDiscountAvailable ? 'cursor-not-allowed' : 'cursor-auto'}`} />
            </div>
...
//             <div className="flex flex-row items-center justify-center gap-4">
//               <label className="w-[200px] text-left font-xl text-gray-700">Rate Applicable:</label>
//               <TextField id="rate" value={formData.rate} variant="outlined" className="w-[210px]" disabled={!allowWriteAccess} required />
//             </div>
...
//             <div className="flex flex-row items-center justify-center gap-4">
//               <label className="w-[200px] text-left font-xl text-gray-700">Payment Received:</label>
//               <TextField id="payment_received" value={formData.payment_received} onChange={handleChange} variant="outlined" disabled={!allowWriteAccess} className={`w-[210px] ${!allowWriteAccess || !isDiscountAvailable ? 'cursor-not-allowed' : 'cursor-auto'}`} />
//             </div>
...
//             <div className="flex justify-center mt-6">
//               <button type="submit" disabled={loading || !allowWriteAccess}
//                 className={`bg-[#1E293B] text-yellow-300 font-semibold px-8 py-2 rounded-md transition ${
//                   allowWriteAccess ? "cursor-pointer hover:bg-[#334155]" : "cursor-not-allowed"
//                 }`} >
