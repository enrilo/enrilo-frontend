...
export default function Sidebar() {
      ...
      subMenu: [
        {
          ...
          path: "/add-new-consultancy",
        }, {
          name: "All Consultancies",
  ...
  useEffect(() => {
    const matched = menuItems.find( (item) => item.path === location.pathname || item.subMenu?.some((sub) => sub.path === location.pathname));
            ...
            {menuItems.map((item, idx) => {
              ...
              return (
                <li key={idx} className="relative group">
                  {hasSubMenu ? (
                    <>
                      <button onClick={() => toggleSubMenu(item.name)} className={`flex items-center justify-between w-full p-2 rounded-md transition-all duration-300 cursor-pointer ${ isMenuActive ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#334155]" }`} >
                      ...
                      <ul className={`ml-10 mt-2 space-y-1 overflow-hidden transition-[max-height] duration-300 ${ activeMenu === item.name && isOpen ? "max-h-64" : "max-h-0" }`} >
                        {item.subMenu.map((sub, subIdx) => (
                          <li key={subIdx}>
                            <Link to={sub.path} className={`flex items-center p-2 rounded-md ${ location.pathname === sub.path ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#334155]" }`} >
                    ...
                    <Link to={item.path} className={`flex items-center p-2 rounded-md ${ isMenuActive ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#334155]" }`}>
                      <span className="flex justify-center w-6">
                        {item.icon}
                      </span>
                      {isOpen && (
