...
//         {/* Header + Menu */}
//         <div className="pb-4 pl-4 pr-4 flex-1 overflow-y-auto flex flex-col justify-between">
//           <div>
...
//   const MenuLink = ({ icon, name, path, isActive }) => (
//     <Link to={path} className={`flex items-center p-2 rounded-md transition-all duration-200 ${ isActive ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#334155]" }`} title={!isOpen ? name : ""} >
//       <span className="flex justify-center w-6">{icon}</span>
//       {isOpen && <span className="ml-3 text-sm">{name}</span>}
//     </Link>
//   );
...
//     <>
//       {/* Mobile Hamburger */}
//       <button className="md:hidden fixed top-3 left-3 z-40 p-2 rounded-md bg-[#1E293B] text-white shadow" onClick={() => setMobileOpen(true)} >
//         <Menu size={24} />
//       </button>

//       {/* Sidebar */}
//       <aside className={`fixed md:static top-0 left-0 h-screen bg-[#1E293B] text-white flex flex-col justify-between transition-all duration-300 z-40 overflow-hidden ${ isOpen ? "w-64" : "w-20" } ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`} >
//         {/* Scrollable container */}
...
//                   {hasSubMenu ? (
//                     <>
//                       <button title={!isOpen ? item.name : ""} onClick={() => toggleSubMenu(item.name)}
//                         className={`flex items-center cursor-pointer justify-between w-full p-2 rounded-md transition-all duration-300 ${
//                           isMenuActive ? "bg-[#475569] text-yellow-300 font-semibold" : "hover:bg-[#334155]"
//                         }`}>
//                         <div className="flex items-center space-x-3 cursor-pointer">
//                           <span>{item.icon}</span>
...
//                       {/* Submenu with smooth open/close */}
//                       <ul className={`ml-10 mt-2 space-y-1 overflow-hidden transition-[max-height] duration-300 ${ activeMenu === item.name && isOpen ? "max-h-64" : "max-h-0" }`}>
//                         {item.subMenu.map((sub, subIdx) => (
//                           <li key={subIdx}>
//                             <MenuLink icon={sub.icon} name={sub.name} path={sub.path} isActive={location.pathname === sub.path} />
//                           </li>
...
//           <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-full bg-[#334155] hover:bg-[#475569] transition cursor-pointer" >
//             {isOpen ? <ChevronLeft size={18} /> : <ChevronRight size={18} />}
//           </button>
//         </div>
