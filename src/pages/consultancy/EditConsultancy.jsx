...
//       const consultancyData = await fetch(`http://localhost:3000/api/consultancies/${params.id}`, {
//         method: "GET",
//         headers: {
...
//       const consultancyJsonData = await consultancyData.json();
//       if (consultancyJsonData.success === false) {
//         return;
//       }
//       console.log(`consultancyJsonData:${JSON.stringify(consultancyJsonData)}`);
...
//             <TextField label="Website" value={formData.website} variant="outlined" onChange={(e) => setFormData((p) => ({...p, website: e.target.value}))} />
...
//             <TextField label="Instagram" value={formData.instagram_url} variant="outlined" onChange={(e) => setFormData((p) => ({...p, instagram_url: e.target.value}))} />
...
export default function EditConsultancy() {
  ...
  useEffect(() =>{
    ...
    const fetchConsultancy = async () => {
      ...
      const consultancyJsonData = await consultancyData.json();
      if (consultancyJsonData.success === false) return;
      ...
      setFormData({
        ...
        instagram_url: fetchedAdmin.instagram_url || "",
        is_single_branch: fetchedAdmin.is_single_branch,
        subdomain: fetchedAdmin.subdomain || "",
        office_details: sanitizedOffices.length ? sanitizedOffices : [{
          office_city: "",
          office_address: "",
          office_type: "Head Office",
          country_code: "",
      ...
      setOriginalSubdomain(fetchedAdmin.subdomain || "");

      setProfilePreviewUrl(fetchedAdmin.photo_url);
      setIsSingleBranch(fetchedAdmin.is_single_branch);
      setBranchType(fetchedAdmin.is_single_branch ? "single" : "multiple");
    };
  ...
  const handleRemoveOffice = useCallback((index) => {
    setFormData((prev) => {
      if (prev.office_details.length === 1) return prev;
      return {
        ...prev,
        office_details: prev.office_details.filter((_, i) => i !== index),
      };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        office_details: cleanedOfficeDetails,
      };

      const res = await fetch(`http://localhost:3000/api/consultancies/${params.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify(payload),
        credentials: "include",
      ...
      setSaveSuccessfulMessage("Consultancy updated successfully! You will now be redirected to All Consultancies Page.");
      setSaveSuccessfulMsgOpen(true);
    } catch (err) {
      console.error(err);
      setFailedToSaveMessage(`Failed to update consultancy: ${err.message}`);
      setFailedToSaveMsgOpen(true);
    } finally {
      setPageLoading(false);
    }
              ...
              <TextField label="Subdomain" value={formData.subdomain} variant="outlined" disabled={!allowWriteAccess}
                ...
                slotProps={{
                  input: {
                    ...
                    endAdornment: (
                      <InputAdornment position="end">.enrilo.com</InputAdornment>
                    ),
                  },
                }}
              />
...
//         instagram_url: fetchedAdmin.instagram_url || "",
//         is_single_branch: fetchedAdmin.is_single_branch,
//         office_details: fetchedAdmin.office_details?.length ? fetchedAdmin.office_details : [
//           {
//             office_city: "",
//             office_address: "",
//             office_type: "Head Office",
...
//       setProfilePreviewUrl(fetchedAdmin.photo_url);
//       setIsSingleBranch(fetchedAdmin.is_single_branch);
//       setBranchType(fetchedAdmin.is_single_branch ? true : false);
//     };
...
//       setSaveSuccessfulMessage("Consultancy updated successfully! You will now be redirected.");
//       setSaveSuccessfulMsgOpen(true);
//     } catch (err) {
//       console.error(err);
//       setFailedToSaveMessage(`Failed to update consultancy: ${err.message}`);
